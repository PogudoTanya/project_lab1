...
function findSubformulas(subFormulas) {
  ...
  for (var index = 0; index < formula.length; index++) {
    if (formula[index] == "(") {
      ...
      findInArray(constants, formula[index]) &&
      !findInArray(subFormulas, formula[index]) &&
      !findInArray(operations, formula[index])
    ) {
...
function start() {
  ...
  var answerContent = document.createTextNode(
    "Answer: " + subFormulas.length + "."
  );
